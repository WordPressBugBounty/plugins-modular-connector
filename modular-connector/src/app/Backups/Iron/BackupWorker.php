...
class BackupWorker
{
    ...
    public function calculateParts(BackupOptions $options)
    {
        ...
        // FIXME Check if job is cancelled
        ...
        Backup::remove($options->name, true);
        $this->deleteAll();

        // Create the backup folder
        Storage::makeDirectory(Backup::path($options->name));
        ...
        foreach ($partTypes as $type => $conf) {
            ...
            $part = new BackupPart($type, $partOptions, $conf['max_depth']);

            if (in_array(BackupOptions::INCLUDE_CORE, $options->included)) {
                $part->calculateMainManifest();
            }
        ...
        \Modular\ConnectorDependencies\dd($parts);
