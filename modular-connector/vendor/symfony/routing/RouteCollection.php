...
class RouteCollection implements \IteratorAggregate, \Countable
{
    ...
    public function add(string $name, Route $route)
    {
        if (\func_num_args() < 3 && __CLASS__ !== static::class && __CLASS__ !== (new \ReflectionMethod($this, __FUNCTION__))->getDeclaringClass()->getName() && !$this instanceof \Modular\ConnectorDependencies\PHPUnit\Framework\MockObject\MockObject && !$this instanceof \Modular\ConnectorDependencies\Prophecy\Prophecy\ProphecySubjectInterface && !$this instanceof \Modular\ConnectorDependencies\Mockery\MockInterface) {
            \Modular\ConnectorDependencies\trigger_deprecation('symfony/routing', '5.1', 'The "%s()" method will have a new "int $priority = 0" argument in version 6.0, not defining it is deprecated.', __METHOD__);
        }
    ...
    public function get(string $name)
    {
        ...
        while (null !== $alias = $this->aliases[$name] ?? null) {
            ...
            if ($alias->isDeprecated()) {
                ...
                \Modular\ConnectorDependencies\trigger_deprecation($deprecation['package'], $deprecation['version'], $deprecation['message']);
            }
            $visited[] = $name;
            $name = $alias->getId();
        }
