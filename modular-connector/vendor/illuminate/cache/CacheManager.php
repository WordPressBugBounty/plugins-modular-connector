...
use Closure;
use Modular\ConnectorDependencies\Illuminate\Contracts\Cache\Factory as FactoryContract;
...
class CacheManager implements FactoryContract
{
    ...
    protected function createDatabaseDriver(array $config)
    {
        ...
        $store = new DatabaseStore($connection, $config['table'], $this->getPrefix($config), $config['lock_table'] ?? 'cache_locks', $config['lock_lottery'] ?? [2, 100]);
    ...
    protected function createDynamodbDriver(array $config)
    {
        ...
        return $this->repository(new DynamoDbStore($client, $config['table'], $config['attributes']['key'] ?? 'key', $config['attributes']['value'] ?? 'value', $config['attributes']['expiration'] ?? 'expires_at', $this->getPrefix($config)));
    }
    /**
    ...
    {
        return \Modular\ConnectorDependencies\tap(new Repository($store), function ($repository) {
