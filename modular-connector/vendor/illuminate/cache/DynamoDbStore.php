...
use Modular\ConnectorDependencies\Aws\DynamoDb\Exception\DynamoDbException;
...
class DynamoDbStore implements LockProvider, Store
{
    ...
    public function many(array $keys)
    {
        ...
        $response = $this->dynamo->batchGetItem(['RequestItems' => [$this->table => ['ConsistentRead' => \false, 'Keys' => \Modular\ConnectorDependencies\collect($prefixedKeys)->map(function ($key) {
            return [$this->keyAttribute => ['S' => $key]];
        })->all()]]]);
        $now = Carbon::now();
        return array_merge(\Modular\ConnectorDependencies\collect(array_flip($keys))->map(function () {
            //
        })->all(), \Modular\ConnectorDependencies\collect($response['Responses'][$this->table])->mapWithKeys(function ($response) use ($now) {
    ...
    public function putMany(array $values, $seconds)
    {
        ...
        $this->dynamo->batchWriteItem(['RequestItems' => [$this->table => \Modular\ConnectorDependencies\collect($values)->map(function ($value, $key) use ($expiration) {
