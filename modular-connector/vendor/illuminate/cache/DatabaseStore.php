...
class DatabaseStore implements LockProvider, Store
{
     ...
     * @return void
     */
    public function __construct(ConnectionInterface $connection, $table, $prefix = '', $lockTable = 'cache_locks', $lockLottery = [2, 100])
    {
        $this->table = $table;
        $this->prefix = $prefix;
    ...
    public function add($key, $value, $seconds)
    {
        ...
        try {
            ...
            return $this->table()->where('key', $key)->where('expiration', '<=', $this->getTime())->update(['value' => $value, 'expiration' => $expiration]) >= 1;
        }
    }
    /**
    ...
    {
        return new DatabaseLock($this->lockConnection ?? $this->connection, $this->lockTable, $this->prefix . $name, $seconds, $owner, $this->lockLottery);
    }
    /**
    ...
    protected function serialize($value)
    {
        ...
        if ($this->connection instanceof PostgresConnection && Str::contains($result, "\x00")) {
