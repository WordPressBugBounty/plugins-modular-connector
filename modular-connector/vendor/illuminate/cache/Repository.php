...
use Modular\ConnectorDependencies\Illuminate\Cache\Events\CacheHit;
use Modular\ConnectorDependencies\Illuminate\Cache\Events\CacheMissed;
use Modular\ConnectorDependencies\Illuminate\Cache\Events\KeyForgotten;
use Modular\ConnectorDependencies\Illuminate\Cache\Events\KeyWritten;
...
class Repository implements ArrayAccess, CacheContract
{
    ...
    public function get($key, $default = null)
    {
        ...
        if (is_null($value)) {
            ...
            $value = \Modular\ConnectorDependencies\value($default);
        } else {
    ...
    {
        $values = $this->store->many(\Modular\ConnectorDependencies\collect($keys)->map(function ($value, $key) {
            return is_string($key) ? $key : $value;
        })->values()->all());
        return \Modular\ConnectorDependencies\collect($values)->map(function ($value, $key) use ($keys) {
    ...
    protected function handleManyResult($keys, $key, $value)
    {
        ...
        if (is_null($value)) {
            $this->event(new CacheMissed($key));
            return isset($keys[$key]) ? \Modular\ConnectorDependencies\value($keys[$key]) : null;
        }
    ...
    {
        return \Modular\ConnectorDependencies\tap($this->get($key, $default), function () use ($key) {
            $this->forget($key);
        });
    }
    /**
    ...
    public function remember($key, $ttl, Closure $callback)
    {
        ...
        $this->put($key, $value = $callback(), \Modular\ConnectorDependencies\value($ttl));
        return $value;
    }
    /**
