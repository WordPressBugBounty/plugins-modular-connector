...
class PruneCommand extends Command
{
    ...
    protected function models()
    {
        ...
        return \Modular\ConnectorDependencies\collect((new Finder())->in($this->getDefaultPath())->files()->name('*.php'))->map(function ($model) {
            ...
            return $namespace . str_replace(['/', '.php'], ['\\', ''], Str::after($model->getRealPath(), realpath(\Modular\ConnectorDependencies\app_path()) . \DIRECTORY_SEPARATOR));
    ...
    {
        return \Modular\ConnectorDependencies\app_path('Models');
    }
    /**
    ...
    {
        $uses = \Modular\ConnectorDependencies\class_uses_recursive($model);
    ...
    {
        $instance = new $model();
        $count = $instance->prunable()->when(in_array(SoftDeletes::class, \Modular\ConnectorDependencies\class_uses_recursive(get_class($instance))), function ($query) {
            $query->withTrashed();
        })->count();
        if ($count === 0) {
            $this->info("No prunable [{$model}] records found.");
