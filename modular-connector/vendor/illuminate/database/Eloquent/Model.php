...
abstract class Model implements Arrayable, ArrayAccess, CanBeEscapedWhenCastToString, HasBroadcastChannel, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    protected static function bootTraits()
    {
        ...
        foreach (\Modular\ConnectorDependencies\class_uses_recursive($class) as $trait) {
            $method = 'boot' . \Modular\ConnectorDependencies\class_basename($trait);
            if (method_exists($class, $method) && !in_array($method, $booted)) {
                forward_static_call([$class, $method]);
                $booted[] = $method;
            }
            if (method_exists($class, $method = 'initialize' . \Modular\ConnectorDependencies\class_basename($trait))) {
    ...
    public function refresh()
    {
        ...
        $this->load(\Modular\ConnectorDependencies\collect($this->relations)->reject(function ($relation) {
            return $relation instanceof Pivot || is_object($relation) && in_array(AsPivot::class, \Modular\ConnectorDependencies\class_uses_recursive($relation), \true);
        })->keys()->all());
        $this->syncOriginal();
        return $this;
    }
    ...
    {
        return $this->table ?? Str::snake(Str::pluralStudly(\Modular\ConnectorDependencies\class_basename($this)));
    }
    /**
    ...
    {
        return Str::snake(\Modular\ConnectorDependencies\class_basename($this)) . '_' . $this->getKeyName();
    }
    /**
    ...
    {
        return str_replace('\\', '.', get_class($this)) . '.{' . Str::camel(\Modular\ConnectorDependencies\class_basename($this)) . '}';
    }
    /**
